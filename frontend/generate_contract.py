...
def haversine(lat1, lon1, lat2, lon2):
    """Calculates ground distance between two GPS coordinates in km"""
    R = 6371.0
...
for sat_name, meta in TARGET_SATS.items():
    ...
    for i in range(0, len(events), 3):
        ...
        # 1. Get the sub-satellite point (where it is on the ground at peak elevation)
        geocentric = satellite.at(t_peak)
        subpoint = wgs84.subpoint(geocentric)
        
        # 2. Calculate if the footprint actually touches the target
        dist_km = haversine(TARGET_LAT, TARGET_LON, subpoint.latitude.degrees, subpoint.longitude.degrees)
        swath_radius = meta["swath"] / 2.0
        
        if dist_km > swath_radius:
            continue # The camera misses the target! Drop the pass.
            
        alt, az, distance = (satellite - blacksburg).at(t_peak).altaz()
        
        cloud_pct = get_cloud_cover_for_time(t_peak.utc_datetime())
        ...
        final_passes.append({
            "pass_id": f"{sat_name.replace(' ', '')}-{t_peak.utc_datetime().strftime('%Y%m%dT%H%M')}",
            "satellite": sat_name,
            "start_utc": t_rise.utc_datetime().strftime('%Y-%m-%dT%H:%M:%SZ'),
            "peak_utc": t_peak.utc_datetime().strftime('%Y-%m-%dT%H:%M:%SZ'), # Crucial for UI
...
output = {
    ...
    "targets": [{
        ...
        "lat": TARGET_LAT,
        "lon": TARGET_LON,
        "passes": sorted(final_passes, key=lambda p: p["peak_utc"])
    }]
}
...
print(f"Contract generated. Found {len(final_passes)} true sensor footprint overlaps.")